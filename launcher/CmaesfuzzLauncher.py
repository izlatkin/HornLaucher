...
def init():
    global SOURCE_PATH, SANDBOX_DIR, CMAESFUZZ_PATH, CMAESFUZZ_TIMEOUT, TESTCOV, CMAESFUZZ_WD, START_WITH
    SANDBOX_DIR = "/home/fmfsu/PyCharm/cmaesfuzz_sandbox/"
    SOURCE_PATH = "/home/fmfsu/Benchs/openssl-simplified"
    CMAESFUZZ_PATH = "/home/fmfsu/Dev/cmaesfuzz/fuzzer.py"
    # PRTEST
    CMAESFUZZ_WD = "/home/fmfsu/Dev/cmaesfuzz/"
    CMAESFUZZ_TIMEOUT = 900
...
def zip_results():
    ...
    try:
        command_executer(zip_command, 2 * CMAESFUZZ_TIMEOUT, 'log.txt')
...
def run_cmaesfuzz(file):
    print("run prtest for {}".format(file))
    save = os.getcwd()
    os.chdir(CMAESFUZZ_WD)
    clean_dir("output")  # clean fusebmc output dir
    cmaesfuzz_command = ["python3.9", CMAESFUZZ_PATH, "--coverage_type", "branch",
                      "--timeout",str(CMAESFUZZ_TIMEOUT), "--write_xml_tests",
                      "--output_dir", "output", file]
    try:
        command_executer(cmaesfuzz_command, 2 * CMAESFUZZ_TIMEOUT, os.path.dirname(file) + '/log.txt')
    except subprocess.CalledProcessError as e:
        logger(os.path.dirname(file) + '/log.txt', [" ".join(cmaesfuzz_command), "FAIL"])


    if (os.path.isdir("output/tests")):
        zip_results()
        if (os.path.isfile("output/tests/tests.zip")):
            shutil.move("output/tests/tests.zip", os.path.dirname(file) + "/tests.zip")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='python script to run cmaesfuzz')
