...
def move_to_sandbox(files, add_h, was_cleaned):
    ...
    for f in files:
        ...
        # todo: workaround for product_lines only (remove later)
        if os.path.isfile("/home/fmfsu/Benchs/sv-benchmarks/c/product-lines/" + basename):
            shutil.copyfile("/home/fmfsu/Benchs/sv-benchmarks/c/product-lines/" + basename,
                            subdir + "/orig_" + basename)
        # todo: workaround for product_lines only (remove later)
...
def get_random_list(n):
    ...
    for i in range(0, n):
        ...
        randomlist.append(n)
    tmp = ",".join([str(i) for i in randomlist])
    return tmp



def generate_random_test_header(keys):
    lines = ["#include <stdio.h>\n", "#include <stdbool.h>\n", "#include <stdlib.h>",
             "void __assert_fail(const char * a, const char * b, unsigned int c, const char * d) {};\n",
             "int precisions = 10;\n"]
    n = 100
    for k in keys:
        lines.append("int cnt_{} = {};\n".format(k, n))
        lines.append("int tot_{} = 0;\n".format(k))

    lines.append("\n\nvoid print_value(int v){\n")
    lines.append("    FILE* f = fopen(\"number.txt\", \"a\");\n")
    lines.append("    if (f != NULL){\n")
    lines.append("      fprintf(f, \"%d \", v);\n")
    lines.append("      fclose(f); f = NULL;\n")
    lines.append("    }\n")
    lines.append("}\n\n")

    for k in keys:
        tmp = "static const int inp_" + str(k) + "[] = {" + get_random_list(n) + "};\n"
        lines.append(tmp)

    lines.append("\n")
    for k in keys:
        lines.append("const int nondet_" + str(k) +"(){\n")
        lines.append("  if (cnt_" + str(k) + " < tot_" + str(k) + "){\n")
        lines.append("    print_value(inp_" + str(k) + "[cnt_" + str(k) + "]);\n")
        lines.append("    return inp_" + str(k) + "[cnt_" + str(k) + "++];}\n")
        lines.append("  else {int rr = rand() % precisions; print_value(rr); return rr;}\n")
        lines.append("}\n\n")
